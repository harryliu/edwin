...
Revised by: Harry Liu, harrychinese@gmail.com
Changes by Harry, 
1. replace extensions argument with file_pattern in constructor,  change file matching method from extension to glob
2. add includeSubFolder argument in constructor, watch log file in given sub folder
3. add excludeFileListFile argument in constructor, pass one file name to specify one excluded log file list in that file
...
class LogWatcher(object):
    ...
    """
        #another example, monitor sub folders
        folder="D:/workspace/trunk/projects/Poc/incr10"
        file_pattern="*.log"
        includeSubFolder=False
        excludeFileListFile="D:/workspace/trunk/projects/Poc/incr10/excludefilelist.txt"
        def callback(filename, lines):
            print(filename, lines)

        watcher = LogWatcher(folder, callback, file_pattern, includeSubFolder, excludeFileListFile, tail_lines=100000)
        watcher.loop()
    """
    ...
    def listdir(self):
        """List directory and filter files by file_pattern.
        You may want to override this to add extra logic or globbing
        support.
        """
        ls = os.listdir(self.folder)
        if self.file_pattern:
            return [x for x in ls if fnmatch.fnmatch(x, self.file_pattern)]
        else:
            return ls


    def listFiles(self, includeSubFolder=False):
        if not includeSubFolder:
            filelist=self.listdir()
            folderlist=[self.folder for f in filelist]
            return zip(folderlist, filelist)
        else:
            filelist=[]
            folderlist=[]
    ...
    def isExcluded(self, absLogFile):
        if self.excludeFileListFile is None:
            return False
        elif not os.path.exists(self.excludeFileListFile):
            return False
        else:
            result=False
            with open(self.excludeFileListFile) as f:
                for line in f:
                    print('log file:'+absLogFile)
                    print('excluded file:'+line)
                    #if os.path.normpath(line)==os.path.normpath(absLogFile):
                    if fnmatch.fnmatch(absLogFile, line):
                        print('This file is excluded,'+absLogFile)
                        result=True
                        break
            return result
    ...
    def update_files(self):
        ls = []
        for folder, name in self.listFiles(self.includeSubFolder):
            absname = os.path.realpath(os.path.join(folder, name))
            if self.isExcluded(absname):
                continue
            try:
                st = os.stat(absname)
